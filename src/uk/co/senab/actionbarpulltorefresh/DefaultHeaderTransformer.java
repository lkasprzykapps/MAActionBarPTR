...
package uk.co.senab.actionbarpulltorefresh;

import uk.co.senab.actionbarpulltorefresh.library.R;
import uk.co.senab.actionbarpulltorefresh.sdk.Compat;
...
public class DefaultHeaderTransformer extends HeaderTransformer {
	...
	private void setupViewsFromStyles(Activity activity, View headerView) {
		...
		// Bar's style (see #93)
		Drawable bg = styleAttrs.hasValue(R.styleable.PullToRefreshHeader_ptrHeaderBackground) ? styleAttrs
				.getDrawable(R.styleable.PullToRefreshHeader_ptrHeaderBackground) : getActionBarBackground(activity);
		if (bg != null) {
		...
		final int titleTextStyle = styleAttrs.getResourceId(R.styleable.PullToRefreshHeader_ptrHeaderTitleTextAppearance,
	...
	private void applyProgressBarSettings() {
		if (mHeaderProgressBar != null) {
			...
			mHeaderProgressBar.setIndeterminateDrawable(new SmoothProgressDrawable.Builder(mHeaderProgressBar.getContext())
					.color(mProgressDrawableColor).strokeWidth(strokeWidth).build());
